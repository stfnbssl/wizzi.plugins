/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.go\.wizzi-override\examples\go_document.js.ittf
*/
'use strict';
...
function executeExample() {
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputPath = path.join(__dirname, 'results', module + '.go.html');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'go/document', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('go.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
