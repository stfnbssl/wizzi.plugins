/*
    ...
    utc time: Thu, 02 Mar 2023 17:43:46 GMT
*/
'use strict';
/**
     go WizziModelFactory
*/
var path = require('path');
var util = require('util');
var _ = require('lodash');
var stringify = require('json-stringify-safe');
var goschema = require('./go-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var gomodel = new gomodelType('EmptyIttfDocument');
        }
        else {
            // Get the model type of the root node of the ittf model.
            var rootNode = mTree.nodes[0];
            var gomodelType = goschema[rootNode.n];
            if (!gomodelType) {
                var maptag = goschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    gomodelType = goschema[maptag];
                }
                if (!gomodelType) {
                    var error = new errors.WizziModelLoadError('In go Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: go', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            gomodel = new gomodelType(rootNode.v);
            gomodel.loadHistory = mTree.loadHistory;
            gomodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                gomodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn go Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new goschema.goContext();
        gomodel.wzInitialize(ctx);
        gomodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In go Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        // _ log.info('Initialized wmt model ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && gomodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(gomodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        gomodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && gomodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
            ...
            callback(null, gomodel);
        });
    }
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'go' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'go' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema go from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
