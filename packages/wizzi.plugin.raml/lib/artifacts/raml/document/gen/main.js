/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.raml\.wizzi-override\lib\artifacts\raml\document\gen\main.js.ittf
*/
'use strict';
...
var myname = 'wizzi.plugin.raml.artifacts.raml.document.gen.main';
...
md.gen = function(model, ctx, callback) {
    ...
    if (model.wzElement !== 'raml') {
        callback(error('InvalidArgument', 'gen', 'Invalid model schema. Expected root element "raml". Received: ' + model.wzElement, model))
    }
    try {
        md.raml(model, ctx, (err, notUsed) => {
...
;
md.raml = function(model, ctx, callback) {
    ctx.w('<raml>');
...
function error(errorName, method, message, model, innerError) {
    return new errors.WizziPluginError(message, model, {
            errorName: errorName, 
            method: 'wizzi.plugin.raml/lib/artifacts/raml/document/gen/main.' + method, 
