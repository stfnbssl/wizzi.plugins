/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.raml\.wizzi-override\examples\raml_document.js.ittf
*/
'use strict';
...
function executeExample() {
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputPath = path.join(__dirname, 'results', module + '.raml.html');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'raml/document', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('raml.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
