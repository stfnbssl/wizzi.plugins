/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.vue\.wizzi-override\lib\artifacts\vue\document\gen\main.js.ittf
*/
'use strict';
...
var myname = 'wizzi.plugin.vue.artifacts.vue.document.gen.main';
...
md.gen = function(model, ctx, callback) {
    ...
    if (model.wzElement !== 'vue') {
        callback(error('InvalidArgument', 'gen', 'Invalid model schema. Expected root element "vue". Received: ' + model.wzElement, model))
    }
    try {
        md.vue(model, ctx, (err, notUsed) => {
...
;
md.vue = function(model, ctx, callback) {
...
md.htmlInclude = function(model, ctx, callback) {
    if (model.get_html) {
        included_writers.writeIncludeHtml(ctx, model, callback)
    }
    else {
...
md.jsInclude = function(model, ctx, callback) {
    if (model.get_js) {
        included_writers.writeIncludeJs(ctx, model, callback)
    }
    else {
...
;
md.tsInclude = function(model, ctx, callback) {
    if (model.get_ts) {
        included_writers.writeIncludeTypescript(ctx, model, callback)
    }
    else {
...
function error(errorName, method, message, model, innerError) {
    return new errors.WizziPluginError(message, model, {
            errorName: errorName, 
            method: 'wizzi.plugin.vue/lib/artifacts/vue/document/gen/main.' + method, 
