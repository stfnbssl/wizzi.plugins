/*
    ...
    utc time: Thu, 07 Nov 2024 16:13:21 GMT
*/
/**
     ai WizziModelFactory
*/
...
var aimTreePreProcessor = require('./ai-mtree-preprocessor.g');

var aischema = require('./ai-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var aimodel = new aimodelType('EmptyIttfDocument');
        }
        ...
        // Load the WizziModel from the root node of the mTree
            ...
            var aimodelType = aischema[rootNode.n];
            if (!aimodelType) {
                var maptag = aischema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    aimodelType = aischema[maptag];
                }
                if (!aimodelType) {
                    var error = new errors.WizziModelLoadError('In ai Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: ai', ittfDocumentUri);
                    return callback(error);
                }
            }
            aimodel = new aimodelType(rootNode.v);
            aimodel.loadHistory = mTree.loadHistory;
            aimodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                aimodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn ai Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        // dump for debug
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && aimodel.toJson && file.isFilePath(ittfDocumentUri)) {
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(aimodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        aimodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            
            // dump for debug
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && aimodel.toJson && file.isFilePath(ittfDocumentUri)) {
            ...
            callback(null, aimodel);
        })
    }
    
    /**
        * Load a WizziModel of type 'ai' from an mTree
    */
    ...
    /**
        // Load a WizziModel of type 'ai' from an IttfDocument uri
        // params
    ...
    */
        ...
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema ai from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
                    ...
                    mTree = aimTreePreProcessor(mTree, loadContext)
                    ;
