/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.ai\.wizzi-override\examples\ai_extended.js.ittf
    utc time: Thu, 20 Feb 2025 12:19:36 GMT
*/
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'ai.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'ai.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'ai.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.ai.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.ai.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.ai.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                ...
                loadModelAndTransform(ittfDocumentUri, {}, "ai/extended", (err, model) => {
                    if (err) {
                        return callback(err);
                    }
