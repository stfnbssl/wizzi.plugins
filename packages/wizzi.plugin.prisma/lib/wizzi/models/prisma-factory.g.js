/*
    ...
    utc time: Sat, 14 Dec 2024 12:38:24 GMT
*/
/**
     prisma WizziModelFactory
*/
...
var prismaschema = require('./prisma-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var prismamodel = new prismamodelType('EmptyIttfDocument');
        }
        ...
        // Load the WizziModel from the root node of the mTree
            ...
            var prismamodelType = prismaschema[rootNode.n];
            if (!prismamodelType) {
                var maptag = prismaschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    prismamodelType = prismaschema[maptag];
                }
                if (!prismamodelType) {
                    var error = new errors.WizziModelLoadError('In prisma Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: prisma', ittfDocumentUri);
                    return callback(error);
                }
            }
            prismamodel = new prismamodelType(rootNode.v);
            prismamodel.loadHistory = mTree.loadHistory;
            prismamodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                prismamodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn prisma Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new prismaschema.prismaContext();
        prismamodel.wzInitialize(ctx);
        prismamodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In prisma Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        ...
        // dump for debug
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && prismamodel.toJson && file.isFilePath(ittfDocumentUri)) {
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(prismamodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        prismamodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            
            // dump for debug
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && prismamodel.toJson && file.isFilePath(ittfDocumentUri)) {
            ...
            callback(null, prismamodel);
        })
    }
    
    /**
        * Load a WizziModel of type 'prisma' from an mTree
    */
    ...
    /**
        // Load a WizziModel of type 'prisma' from an IttfDocument uri
        // params
    ...
    */
        ...
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema prisma from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
