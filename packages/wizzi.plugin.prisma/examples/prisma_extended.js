/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.prisma\.wizzi-override\examples\prisma_extended.js.ittf
    utc time: Wed, 15 May 2024 12:22:04 GMT
*/
'use strict';
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'prisma.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'prisma.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'prisma.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.prisma.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.prisma.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.prisma.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                ...
                loadModelAndTransform(ittfDocumentUri, {}, "prisma/extended", (err, model) => {
