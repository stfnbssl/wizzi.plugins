/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.app\.wizzi-override\examples\app_extended.js.ittf
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'app.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'app.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'app.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.app.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.app.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.app.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                ...
                loadModelAndTransform(ittfDocumentUri, {}, "app/extended", (err, model) => {
                    if (err) {
                        return callback(err);
                    }
