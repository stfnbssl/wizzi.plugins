/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.toml\.wizzi-override\examples\toml_document.js.ittf
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'toml/document.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'toml/document.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'toml/document.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, wzCtx, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputPath = path.join(__dirname, 'results', 'toml', module + '-document.g.toml');
            loadWizziModel(ittfDocumentUri, {
                ...
                loadModelAndGenerateArtifact(ittfDocumentUri, {
                    wzCtx: wzCtx
                 }, 'toml/document', (err, artifactText) => {
