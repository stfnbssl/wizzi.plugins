...
var pandocmTreePreProcessor = require('./pandoc-mtree-preprocessor.g');

var pandocschema = require('./pandoc-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var pandocmodel = new pandocmodelType('EmptyIttfDocument');
        }
        ...
        // Load the WizziModel from the root node of the mTree
            ...
            var pandocmodelType = pandocschema[rootNode.n];
            if (!pandocmodelType) {
                var maptag = pandocschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    pandocmodelType = pandocschema[maptag];
                }
                if (!pandocmodelType) {
                    var error = new errors.WizziModelLoadError('In pandoc Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: pandoc', ittfDocumentUri);
                    return callback(error);
                }
            }
            pandocmodel = new pandocmodelType(rootNode.v);
            pandocmodel.loadHistory = mTree.loadHistory;
            pandocmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                pandocmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn pandoc Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new pandocschema.pandocContext();
        pandocmodel.wzInitialize(ctx);
        pandocmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In pandoc Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        ...
        // dump for debug
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && pandocmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(pandocmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        pandocmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            
            // dump for debug
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && pandocmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            ...
            callback(null, pandocmodel);
        })
    }
    
    /**
        * Load a WizziModel of type 'pandoc' from an mTree
    */
    ...
    /**
        // Load a WizziModel of type 'pandoc' from an IttfDocument uri
        // params
    ...
    */
        ...
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema pandoc from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
                    ...
                    mTree = pandocmTreePreProcessor(mTree, loadContext)
                    ;
