/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.pandoc\.wizzi-override\examples\pandoc_extended.js.ittf
    utc time: Tue, 30 May 2023 13:50:12 GMT
*/
'use strict';
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'pandoc.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'pandoc.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'pandoc.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.pandoc.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.pandoc.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.pandoc.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                ...
                loadModelAndTransform(ittfDocumentUri, {}, "pandoc/extended", (err, model) => {
