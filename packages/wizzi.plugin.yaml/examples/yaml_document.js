/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.yaml\.wizzi-override\examples\yaml_document.js.ittf
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'yaml/document.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'yaml/document.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'yaml/document.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('yaml/document.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.yaml.ittf');
            var outputPath = path.join(__dirname, 'results', 'yaml', module + '-document.g.yaml');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'yaml/document', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('yaml/document.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
