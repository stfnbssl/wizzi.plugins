/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.graphql\.wizzi-override\examples\graphql_cheatsheetFolder.js.ittf
    utc time: Mon, 06 May 2024 14:25:16 GMT
*/
'use strict';
...
function executeExample() {
    createPluginFactory((err, plugin) => {
    
        if (err) {
            return callback(err);
        }
        plugin.getCheatsheetFolder('ittf', (err, csFolder) => {
        
            if (err) {
                return callback(err);
            }
            console.log("csFolder", csFolder, __filename);
        }
        )
    }
    )
