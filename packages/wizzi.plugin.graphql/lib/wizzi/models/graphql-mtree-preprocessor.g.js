/*
    ...
    utc time: Sat, 20 Apr 2024 04:16:40 GMT
*/
'use strict';
    ...
    if (node.n === '"""') {
        if (node.children.length > 0) {
            ...
            for (i=0; i<i_len; i++) {
                ...
                childrenTexts.push(buildTextTree(item))
            }
            node.children = childrenTexts;
        }
    }
}
function buildTextTree(node) {
    const textNode = {
        n: '+', 
        v: node.n + ' ' + node.v, 
        children: [
    ...
    copyNodeAttrs(node, textNode)
    var i, i_items=node.children, i_len=node.children.length, item;
    for (i=0; i<i_len; i++) {
        item = node.children[i];
        textNode.children.push(buildTextTree(item))
    }
    return textNode;
}
