...
var selectionSet = (function (basenode) {
    ...
    selectionSet.prototype.loadChild = function(child) {
        ...
        if (name === '_') {
            return this.wzLoadToChildColl(child, _md.functionField, this.selections);
        }
        if (name === '{') {
            return this.wzLoadOneToOne(child, _md.selectionSet, 'selectionSet');
        }
...
var functionField = (function (selection) {
    _inherits(functionField, selection);
    function functionField(name, sourceLineInfo) {
        _get(Object.getPrototypeOf(functionField.prototype), 'constructor', this).call(this, name,sourceLineInfo);
        _classCallCheck(this, functionField);
        this.wzElement = "functionField";
        // relation argument
        this.xarguments = [];
        // relation selectionSet
        this.selectionSet = null;
    }
    functionField.prototype.getArgument = function(name) {
        var found = null;
        this.xarguments.forEach(function(item) {
            found = found || (item.wzName === name ? item : null);
        })
    ...
    functionField.prototype.toJson = function() {
        var result = {};
        this.toJsonProperties(result);
        this.toJsonChildren(result);
        return result;
    }
    functionField.prototype.toJsonProperties = function(result) {
        _md.selection.prototype.toJsonProperties.call(this, result);
        if (this.__jsonProps) {
            for (var k in this.__jsonProps) {
                result[k] = this.__jsonProps[k];
    ...
    functionField.prototype.toJsonChildren = function(result) {
        _md.selection.prototype.toJsonChildren.call(this, result);
        var items, item;
        items = [];
        this.xarguments.forEach(item => 
    ...
    functionField.prototype.loadChild = function(child) {
        var ok = false, name = child.n.toLowerCase();
        if (name === '@') {
            return this.wzLoadToChildColl(child, _md.genericArgument, this.xarguments);
        }
    ...
    functionField.prototype.loadFromNode = function(node) {
        node.children.forEach((item) => {
    ...
    functionField.prototype.wzVerify = function(ctx) {
        this.xarguments.forEach(item => 
    ...
    functionField.prototype.wzInitialize = function(ctx) {
        this.xarguments.forEach(item => 
...
_md.functionField = functionField;
...
var stringVar = (function (variable) {
    ...
    stringVar.prototype.wzInitialize = function(ctx) {
        ...
        /*
            ...
            utc time: Sat, 13 Apr 2024 11:30:37 GMT
        */
        this.type = 'String';
...
var interfaceTypeDef = (function (typeDef) {
    ...
    interfaceTypeDef.prototype.addImplement = function(name, sourceLineInfo) {
        var retval = new _md.implement(name, sourceLineInfo);
        retval.wzParent = this;
        this.ximplements.push(retval);
        return retval;
    }
    interfaceTypeDef.prototype.getImplement = function(name) {
        var found = null;
        this.ximplements.forEach(function(item) {
            found = found || (item.wzName === name ? item : null);
        })
...
_md.__tagElementMapping = { '+': 'text', '#': 'comment', '%': 'genericDirective', '%list': 'isListDirective', '%required': 'isRequiredDirective', '%deprecated': 'isDeprecatedDirective', '%skip': 'skipDirective', '%include': 'includeDirective', ':schema': 'schemaDef', ':query': 'queryType', ':mutation': 'mutationType', ':subscription': 'subscriptionType', ':enum': 'enumType', ':scalar': 'scalarTypeDef', ':type': 'objectTypeDef', ':interface': 'interfaceTypeDef', ':union': 'unionTypeDef', '': 'unionMemberType', ':input': 'inputObjectTypeDef', 'type': 'objectTypeDef', '@': 'inputValueDef', '.': 'fieldDef', '{': 'selectionSet', 'on': 'directiveLocation', 'if': 'ifArgument', 'string': 'stringVar', 'int': 'intVar', 'float': 'floatVar', 'boolean': 'booleanVar', '_': 'functionField', '...': 'fragmentSpread', '...on': 'inlineFragment', 'ID': 'IDValue', 'enum': 'enumValue', '[': 'arrayValue', '\"\"\"': 'undefined', '&': 'implement', ':string': 'stringNameType', ':int': 'intNameType', ':float': 'floatNameType', ':boolean': 'booleanNameType', ':Date': 'DateNameType', ':ref': 'refNameType' };
