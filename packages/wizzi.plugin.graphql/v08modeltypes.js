/*
    ...
    utc time: Wed, 10 Apr 2024 14:59:49 GMT
*/
'use strict';
...
function executeExample() {
    ...
    const moduleName = arg && arg.length > 0 ? arg : 'graphql';
    executeGenerateModules([
        moduleName
    ], (err, result) => {
    
        if (err) {
            console.log("[31m%s[0m", 'v08modeltypes.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'v08modeltypes.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'v08modeltypes.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
            executeGenerateFactories([
                moduleName
            ], (err, result) => {
            
                if (err) {
                    console.log("[31m%s[0m", 'v08modeltypes.executeGenerateFactories.err', err);
                    console.log("[31m%s[0m", 'v08modeltypes.executeGenerateFactories.err.toString()', err.toString());
                    if (err.inner) {
                        console.log("[31m%s[0m", 'v08modeltypes.executeGenerateFactories.err.inner.toString()', err.inner.toString());
                    }
                }
                else {
                }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('v08modeltypes.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, '.wizzi-override', 'lib', 'wizzi', 'schemas', module + '.wfschema.ittf');
            var outputPath = path.join(__dirname, 'lib', 'wizzi', 'models', module + '-model.g.js');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'wfschema/model', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('v08modeltypes.executeGenerateModules.outputPath: ' + outputPath, __filename);
    ...
    function executeGenerateFactories(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
        
            console.log('v08modeltypes.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, '.wizzi-override', 'lib', 'wizzi', 'schemas', module + '.wfschema.ittf');
            var outputPath = path.join(__dirname, 'lib', 'wizzi', 'models', module + '-factory.g.js');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'wfschema/factory', (err, artifactText) => {
    ...
    wizzi.fsFactory({
        plugins: {
            items: [
                ...
                '../../wizzi/packages/wizzi-js/index.js'
            ], 
...
function executeWizziJob_2(wzjobDocumentUri, options) {
    ...
    wizzi.executeWizziJob({
        ...
        if (err) {
            wizzi.printWizziJobError('wfschema', err);
        }
    })
}
