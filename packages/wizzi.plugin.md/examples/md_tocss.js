/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.md\.wizzi-override\examples\md_tocss.js.ittf
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'md/tocss.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'md/tocss.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'md/tocss.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('md/tocss.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.md.ittf');
            var outputPath = path.join(__dirname, 'results', 'md', module + '-tocss.g.css');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'md/tocss', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('md/tocss.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
