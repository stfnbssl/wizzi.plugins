/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.logbot\.wizzi-override\examples\logbot_extended.js.ittf
    utc time: Tue, 17 Dec 2024 17:22:40 GMT
*/
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'logbot.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'logbot.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'logbot.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.logbot.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.logbot.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.logbot.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                ...
                loadModelAndTransform(ittfDocumentUri, {}, "logbot/extended", (err, model) => {
                    if (err) {
                        return callback(err);
                    }
