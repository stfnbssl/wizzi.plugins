/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.logbot\.wizzi-override\examples\logbot_document.js.ittf
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'logbot/document.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'logbot/document.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'logbot/document.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, wzCtx, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputPath = path.join(__dirname, 'results', 'logbot', module + '-document.g.logbot');
            loadWizziModel(ittfDocumentUri, {
                ...
                loadModelAndGenerateArtifact(ittfDocumentUri, {
                    wzCtx: wzCtx
                 }, 'logbot/document', (err, artifactText) => {
                    if (err) {
                        return callback(err);
                    }
