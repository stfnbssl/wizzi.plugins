/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.docs\.wizzi-override\examples\docs_tohast.js.ittf
    utc time: Wed, 13 Mar 2024 07:01:22 GMT
*/
'use strict';
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'docs.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'docs.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'docs.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.docs.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.docs.json');
            var outputTohastPath = path.join(__dirname, 'results', module + '.docs.tohast.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                ...
                loadModelAndTransform(ittfDocumentUri, {}, "docs/tohast", (err, model) => {
                
                    if (err) {
                        return callback(err);
                    }
                    file.write(outputTohastPath, stringify(model, null, 4))
