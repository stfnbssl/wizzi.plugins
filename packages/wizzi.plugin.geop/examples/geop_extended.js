/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.geop\.wizzi-override\examples\geop_extended.js.ittf
    utc time: Wed, 13 Mar 2024 07:01:31 GMT
*/
'use strict';
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'geop.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'geop.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'geop.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.geop.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.geop.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.geop.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                ...
                loadModelAndTransform(ittfDocumentUri, {}, "geop/extended", (err, model) => {
