...
'use strict';
/**
     org WizziModelFactory
*/
var path = require('path');
var util = require('util');
var _ = require('lodash');
var stringify = require('json-stringify-safe');
var orgschema = require('./org-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var orgmodel = new orgmodelType('EmptyIttfDocument');
        }
        else {
            // Get the model type of the root node of the ittf model.
            var rootNode = mTree.nodes[0];
            var orgmodelType = orgschema[rootNode.n];
            if (!orgmodelType) {
                var maptag = orgschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    orgmodelType = orgschema[maptag];
                }
                if (!orgmodelType) {
                    var error = new errors.WizziModelLoadError('In org Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: org', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            orgmodel = new orgmodelType(rootNode.v);
            orgmodel.loadHistory = mTree.loadHistory;
            orgmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                orgmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn org Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new orgschema.orgContext();
        orgmodel.wzInitialize(ctx);
        orgmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In org Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        // _ log.info('Initialized wmt model ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && orgmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(orgmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        orgmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && orgmodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
            ...
            callback(null, orgmodel);
        });
    }
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'org' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'org' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema org from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
