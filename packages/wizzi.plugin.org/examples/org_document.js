/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.org\.wizzi-override\examples\org_document.js.ittf
*/
'use strict';
...
function executeExample() {
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'org.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'org.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'org.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('org.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.org.ittf');
            var outputPath = path.join(__dirname, 'results', module + '.org.html');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'org/document', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('org.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
...
function executeWizziJob_2(wfjobDocumentUri, options) {
    ...
    wizzi.executeWizziJob({
        ...
        if (err) {
            wizzi.printWizziJobError('org', err);
        }
    })
}
