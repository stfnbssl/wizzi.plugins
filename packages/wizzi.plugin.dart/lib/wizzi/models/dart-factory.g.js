...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var dartmodel = new dartmodelType('EmptyIttfDocument');
        }
        ...
        // Load the WizziModel from the root node of the mTree
            ...
            var dartmodelType = dartschema[rootNode.n];
            if (!dartmodelType) {
                var maptag = dartschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    dartmodelType = dartschema[maptag];
                }
                if (!dartmodelType) {
                    var error = new errors.WizziModelLoadError('In dart Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: dart', ittfDocumentUri);
                    return callback(error);
                }
            }
            dartmodel = new dartmodelType(rootNode.v);
            dartmodel.loadHistory = mTree.loadHistory;
            dartmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                dartmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn dart Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new dartschema.dartContext();
        dartmodel.wzInitialize(ctx);
        dartmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In dart Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        ...
        // dump for debug
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && dartmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(dartmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        dartmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            
            // dump for debug
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && dartmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            ...
            callback(null, dartmodel);
        })
    }
    
    /**
        * Load a WizziModel of type 'dart' from an mTree
    */
    ...
    /**
        // Load a WizziModel of type 'dart' from an IttfDocument uri
        // params
    ...
    */
        ...
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema dart from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
