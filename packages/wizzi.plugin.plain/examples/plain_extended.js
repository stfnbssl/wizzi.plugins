/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.plain\.wizzi-override\examples\plain_extended.js.ittf
    utc time: Fri, 09 Jun 2023 11:01:01 GMT
*/
'use strict';
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'plain.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'plain.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'plain.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.plain.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.plain.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.plain.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                ...
                loadModelAndTransform(ittfDocumentUri, {}, "plain/extended", (err, model) => {
