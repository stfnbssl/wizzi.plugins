...
var plainschema = require('./plain-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var plainmodel = new plainmodelType('EmptyIttfDocument');
        }
        ...
        // Load the WizziModel from the root node of the mTree
            ...
            var plainmodelType = plainschema[rootNode.n];
            if (!plainmodelType) {
                var maptag = plainschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    plainmodelType = plainschema[maptag];
                }
                if (!plainmodelType) {
                    var error = new errors.WizziModelLoadError('In plain Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: plain', ittfDocumentUri);
                    return callback(error);
                }
            }
            plainmodel = new plainmodelType(rootNode.v);
            plainmodel.loadHistory = mTree.loadHistory;
            plainmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                plainmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn plain Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new plainschema.plainContext();
        plainmodel.wzInitialize(ctx);
        plainmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In plain Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        ...
        // dump for debug
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && plainmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(plainmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        plainmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            
            // dump for debug
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && plainmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            ...
            callback(null, plainmodel);
        })
    }
    
    /**
        * Load a WizziModel of type 'plain' from an mTree
    */
    ...
    /**
        // Load a WizziModel of type 'plain' from an IttfDocument uri
        // params
    ...
    */
        ...
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema plain from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
