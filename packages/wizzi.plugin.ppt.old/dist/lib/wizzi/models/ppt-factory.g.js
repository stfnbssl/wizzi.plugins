/*
    ...
    utc time: Fri, 01 Jul 2022 16:43:05 GMT
*/
'use strict';
/**
...
var pptmTreePreProcessor = require('./ppt-mtree-preprocessor.g');
var pptschema = require('./ppt-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var pptmodel = new pptmodelType('EmptyIttfDocument');
        }
        else {
            // Get the model type of the root node of the ittf model.
            var rootNode = mTree.nodes[0];
            var pptmodelType = pptschema[rootNode.n];
            if (!pptmodelType) {
                var maptag = pptschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    pptmodelType = pptschema[maptag];
                }
                if (!pptmodelType) {
                    var error = new errors.WizziModelLoadError('In ppt Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: ppt', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            pptmodel = new pptmodelType(rootNode.v);
            pptmodel.loadHistory = mTree.loadHistory;
            pptmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                pptmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn ppt Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new pptschema.pptContext();
        pptmodel.wzInitialize(ctx);
        pptmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In ppt Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        // _ log.info('Initialized wmt model ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && pptmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(pptmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        pptmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && pptmodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
            ...
            callback(null, pptmodel);
        });
    }
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'ppt' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'ppt' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema ppt from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
                    ...
                    mTree = pptmTreePreProcessor(mTree, loadContext);
