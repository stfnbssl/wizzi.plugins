function docs_Generics_Introduction() {
    /*
        Generics
        Introduction
        A major part of software engineering is building components that not only have well-defined and consistent APIs,
        but are also reusable. Components that are capable of working on the data of today as well as the data of tomorrow
    ...
    /* We’ve now added a type variable T to the identity function. This T allows us to capture the type the user provides (e.g. number),
        ...
        Unlike using any, it’s also just as precise (ie, it doesn’t lose any information) as the first identity function
    ...
    /* Notice that we didn’t have to explicitly pass the type in the angle brackets (<>);
        ...
        you may need to explicitly pass in the type arguments as we did in the previous example when the compiler fails
        to infer the type, as may happen in more complex examples.*/
}
function docs_Generics_Type_Variables() {
    /*
    ...
    /* When we do, the compiler will give us an error that we’re using the .length member of arg,
        but nowhere have we said that arg has this member. Remember, we said earlier that these type variables
        stand in for any and all types, so someone using this function could have passed in a number instead,
    ...
    /* You can read the type of loggingIdentity as “the generic function loggingIdentity takes a type parameter T,
        ...
        as T would bind to number. This allows us to use our generic type variable T as part of the types we’re working with,
...
function docs_Generics_Types() {
    ...
    let myIdentity: GenericIdentityFn2<number> = identity5;
    ...
    /*
        ...
        Understanding when to put the type parameter directly on the call signature and when to put it on the interface
        itself will be helpful in describing what aspects of a type are generic.
        In addition to generic interfaces, we can also create generic classes. Note that it is not possible to create generic enums and namespaces.*/
}
function docs_Generics_Classes() {
    /*
        Generic Classes
    ...
    /*
        ...
        it to only use the number type.
...
function docs_Generics_Constraints() {
    /*
        Generic Constraints
        If you remember from an earlier example, you may sometimes want to write a generic function that works on a set of types
...
function docs_Generics_ClassTypes() {
    /*
    ...
    /* A more advanced example uses the prototype property to infer and constrain relationships between the constructor function
    ...
    var x = createInstance(Lion).keeper.nametag;  // typechecks!
    var y = createInstance(Bee).keeper.hasMask;   // typechecks!
}