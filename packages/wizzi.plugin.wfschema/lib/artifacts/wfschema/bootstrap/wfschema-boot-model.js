/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.wfschema\.wizzi-override\lib\artifacts\wfschema\bootstrap\wfschema-boot-model.js.ittf
...
var Declare = (function (Node) {
    ...
    Declare.prototype.importStatement = function(statement) {
        ...
        if (statement.get_js) {
            ...
            _codeLines = statement.__codeLines || [];
        }
...
var Method = (function (Node) {
    ...
    function Method(name) {
        ...
        console.log('method.ctor', name, this.wzName, __filename);
        this.emitKey = null;
        this.isStatic = false;
    ...
    Method.prototype.importStatement = function(statement) {
        var _statement = statement;
        var _codeLines = [];
        console.log('================> method.name.importStatement.statement.get_js', this.wzName, statement.wzName, this.wzParent && this.wzParent.wzName, !!(statement.get_js), __filename);
        if (statement.get_js) {
    ...
    Method.prototype.stringifyWizziStatement = function(wizziNode, parent) {
        console.log('stringifyWizziStatement', wizziNode.wzTag, wizziNode.wzName, wizziNode.statements, __filename);
        if (!wizziNode.wzTag && !wizziNode.wzName && wizziNode.statements) {
            return ;
        }
        var json = {
            n: wizziNode.wzTag, 
...
var WizziSchema = (function (Node) {
    ...
    */
                ...
                ctx.wizziFactory.generateArtifact(wizziModel, 'generated from wfschema model', 'js/module', {}, (err, artifactText) => {
                    ...
                    for (i=0; i<i_len; i++) {
                        ...
                        if (line.trim() == "'use strict';") {
                            ...
                            item.__codeLines.push(line)
                        }
                    }
                    return callback(null, item.__codeLines);
                }
                )
            }
            )
    ...
    function collectJsIncludes(wfschemaWizziModel) {
        const ret = [];
        var i, i_items=wfschemaWizziModel.declares, i_len=wfschemaWizziModel.declares.length, declareFrom;
        for (i=0; i<i_len; i++) {
            declareFrom = wfschemaWizziModel.declares[i];
            var j, j_items=declareFrom.statements, j_len=declareFrom.statements.length, statementFrom;
            for (j=0; j<j_len; j++) {
                statementFrom = declareFrom.statements[j];
    ...
    WizziSchema.prototype.toJson = function() {
        return {
                name: this.name, 
