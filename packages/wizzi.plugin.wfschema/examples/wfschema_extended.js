/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.wfschema\.wizzi-override\examples\wfschema_extended.js.ittf
...
function executeExample() {
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'wfschema.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'wfschema.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'wfschema.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('wfschema.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.wfschema.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.wfschema.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.wfschema.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('wfschema.example.executeGenerateModules.outputLoadedPath: ' + outputLoadedPath, __filename);
                file.write(outputLoadedPath, stringify(model.toJson(), null, 4))
                loadModelAndTransform(ittfDocumentUri, {}, "wfschema/extended", (err, model) => {
                
                    if (err) {
                        return callback(err);
                    }
                    console.log('wfschema.example.executeGenerateModules.outputExtendedPath: ' + outputExtendedPath, __filename);
