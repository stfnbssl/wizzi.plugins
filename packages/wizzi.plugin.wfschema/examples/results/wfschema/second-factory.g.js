...
var secondschema = require('./second-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var secondmodel = new secondmodelType('EmptyIttfDocument');
        }
        ...
        // Load the WizziModel from the root node of the mTree
            ...
            var secondmodelType = secondschema[rootNode.n];
            if (!secondmodelType) {
                var maptag = secondschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    secondmodelType = secondschema[maptag];
                }
                if (!secondmodelType) {
                    var error = new errors.WizziModelLoadError('In second Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: second', ittfDocumentUri);
                    return callback(error);
                }
            }
            secondmodel = new secondmodelType(rootNode.v);
            secondmodel.loadHistory = mTree.loadHistory;
            secondmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                secondmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn second Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new secondschema.secondContext();
        secondmodel.wzInitialize(ctx);
        secondmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In second Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        ...
        // dump for debug
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && secondmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(secondmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        secondmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            
            // dump for debug
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && secondmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            ...
            callback(null, secondmodel);
        })
    }
    
    /**
        * Load a WizziModel of type 'second' from an mTree
    */
    ...
    /**
        // Load a WizziModel of type 'second' from an IttfDocument uri
        // params
    ...
    */
        ...
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema second from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
