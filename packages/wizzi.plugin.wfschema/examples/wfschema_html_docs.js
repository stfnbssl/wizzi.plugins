/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.wzschema\.wizzi\examples\wfschema_html_docs.js.ittf
*/
'use strict';
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'wfschema/html_docs.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'wfschema/html_docs.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'wfschema/html_docs.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('wfschema/html_docs.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.wfschema.ittf');
            var outputPath = path.join(__dirname, 'results', 'wfschema', module + '-html_docs.g.html');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'wfschema/html_docs', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('wfschema/html_docs.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
