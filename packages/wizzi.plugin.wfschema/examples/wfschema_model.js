/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.wzschema\.wizzi\examples\wfschema_model.js.ittf
*/
'use strict';
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'wfschema/model.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'wfschema/model.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'wfschema/model.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'wfschema/model', (err, artifactText) => {
                ...
                console.log('wfschema/model.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
