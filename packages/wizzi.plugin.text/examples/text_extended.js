/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.text\.wizzi-override\examples\text_extended.js.ittf
    utc time: Wed, 13 Mar 2024 07:14:43 GMT
*/
'use strict';
...
function executeExample() {
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'text.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'text.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'text.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('text.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.text.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.text.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.text.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('text.example.executeGenerateModules.outputLoadedPath: ' + outputLoadedPath, __filename);
                file.write(outputLoadedPath, stringify(model.toJson(), null, 4))
                loadModelAndTransform(ittfDocumentUri, {}, "text/extended", (err, model) => {
                
                    if (err) {
                        return callback(err);
                    }
                    console.log('text.example.executeGenerateModules.outputExtendedPath: ' + outputExtendedPath, __filename);
...
function executeWizziJob_2(wfjobDocumentUri, options) {
    ...
    wizzi.executeWizziJob({
        ...
        if (err) {
            wizzi.printWizziJobError('text', err);
        }
    })
}
