/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.text\.wizzi-override\examples\text_document.js.ittf
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'text/document.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'text/document.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'text/document.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('text/document.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.text.ittf');
            var outputPath = path.join(__dirname, 'results', 'text', module + '-document.g.txt');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'text/document', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('text/document.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
