/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.c\.wizzi-override\examples\c_document.js.ittf
...
function executeExample() {
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputPath = path.join(__dirname, 'results', module + '.c.html');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'c/document', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('c.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
