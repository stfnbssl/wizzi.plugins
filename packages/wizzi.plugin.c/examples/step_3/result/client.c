...
BOOL ParseArguments(int argc, char *argv[]) {
    ...
    */
    ...
    int i;
    printf("Entering ParseArguments()");printf("\n");
    ...
    for (i = 1; i < argc ; i++) {
        ...
        if (!(firstChar == '-' || firstChar == '/')) {
            printf("ERROR: Option has to begin with - or / : %s", argv[i]); printf("\n");
            PrintUsage (argv[0]);
            goto CLEANUP;
        }
        // process the option.
        switch (argv[i][1]) {
            case 'a':
                ...
                if (i + 1 >= argc) {
                    printf("ERROR: Argument 0/4/6 needed for -a option"); printf("\n");
                    PrintUsage (argv[0]);
                    goto CLEANUP;
                }
                ...
                switch (atoi(argv[i+1])) {
                    ...
                    // Invalid value.
                    default: {
                        printf("ERROR: Invalid address family. Must be 0/4/6"); printf("\n");
                        PrintUsage (argv[0]);
                        goto CLEANUP;
                    }
                }
            ...
            case 'n':
                ...
                // first check if there's one more argument.
                if (i + 1 >= argc) {
                    printf("ERROR: Server name needed for -n option"); printf("\n");
                    PrintUsage (argv[0]);
                    goto CLEANUP;
                }
                ...
                if (FAILED(StringCchLength(argv[i+1],INET6_ADDRSTRLEN, NULL))) {
                    printf("ERROR: Server name string too long. can't exceed %d characters", INET6_ADDRSTRLEN); printf("\n");
                    PrintUsage (argv[0]);
                    goto CLEANUP;
                }
            ...
            case 'e':
                ...
                // first check if there's one more argument.
                if (i + 1 >= argc) {
                    printf("ERROR: Port number needed for -e option"); printf("\n");
                    PrintUsage (argv[0]);
                    goto CLEANUP;
                }
                // make sure the input string length is less than
                ...
                if (FAILED(StringCchLength(argv[i+1], NI_MAXSERV, NULL))) {
                    printf("ERROR: Port number too long. can't exceed %d characters", NI_MAXSERV); printf("\n");
                    PrintUsage (argv[0]);
                    goto CLEANUP;
                }
            ...
            case 'b':
                ...
                // first check if there's one more argument.
                if (i + 1 >= argc) {
                    printf("ERROR: Number of bytes needed for -b option"); printf("\n");
                    PrintUsage (argv[0]);
                    goto CLEANUP;
                }
                ...
                if (g_ClientContext.sendBufSize < 0) {
                    printf("ERROR: Number of bytes must be > 0"); printf("\n");
                    PrintUsage (argv[0]);
                    goto CLEANUP;
                }
                // Continue.
            ...
            case 'l':
                ...
                if (i + 1 < argc) {
                    ...
                    if (g_ClientContext.loopCount < 1) {
                        printf("ERROR: Loop Count must be > 0 : %d", g_ClientContext.loopCount); printf("\n");
                        PrintUsage (argv[0]);
                        goto CLEANUP;
                    }
                    // indicate that we have processed the next argument as well.
            ...
            case 'd':
                ...
                // first check if there's one more argument.
                if (i + 1 >= argc) {
                    printf("ERROR: Number of milliseconds needed for -d option"); printf("\n");
                    PrintUsage (argv[0]);
                    goto CLEANUP;
                }
            ...
            case 's':
                ...
                // first check if there's one more argument.
                if (i + 1 >= argc) {
                    printf("ERROR: Scenario Number needed for -s option"); printf("\n");
                    PrintUsage (argv[0]);
                    goto CLEANUP;
                }
                ...
                if (g_ClientContext.scenario < 1 || g_ClientContext.scenario > SC_NUM_SCENARIOS) {
                    printf("ERROR: Invalid scenario type: %d. Must be between 1 to %d",g_ClientContext.scenario, SC_NUM_SCENARIOS); printf("\n");
                    PrintUsage (argv[0]);
                    goto CLEANUP;
                }
                // Continue.
            ...
            case '?':
                ...
                goto CLEANUP;
            default: {
                printf("ERROR: Unrecognized option: %s", argv[i]); printf("\n");
                PrintUsage (argv[0]);
                goto CLEANUP;
            }
        }
    ...
    */
    printf("Parsed input arguments. The following values will be used :");printf("\n");
    printf("\tAddress Family = %s", __AFImage(g_ClientContext.addressFamily));printf("\n");
    printf("\tServer = %s",g_ClientContext.szServer);printf("\n");
    printf("\tPort = %s", g_ClientContext.szPort);printf("\n");
    printf("\tBytes to Send = %ld", g_ClientContext.sendBufSize);printf("\n");
    printf("\tReceive buffer size = %ld", g_ClientContext.recvBufSize);printf("\n");
    printf("\tIterations  = %ld", g_ClientContext.loopCount);printf("\n");
    printf("\tDelay = %ld", g_ClientContext.delay);printf("\n");
    printf("\tScenario = %d", g_ClientContext.scenario);printf("\n");
    // all went well, signal that we can proceed.
    retVal = TRUE;
    
    CLEANUP:
        printf("Exiting ParseArguments()");printf("\n");
        return retVal;
}
...
void PrintAddressString(LPSOCKADDR pSockAddr, DWORD dwSockAddrLen) {
    ...
    if (WSAAddressToString(pSockAddr, dwSockAddrLen, NULL, buf, &dwBufSize) == SOCKET_ERROR) {
        printf("ERROR: WSAAddressToString failed %d", WSAGetLastError());printf("\n");
        goto CLEANUP;
    }
    printf("%s\n", buf);printf("\n");
...
SOCKET CreateClientSocket() {
    ...
    printf("Entering CreateClientSocket()");printf("\n");
    ...
    if (res == NULL) {
        printf("getaddrinfo returned res = NULL"); printf("\n");
        goto CLEANUP;
    }
    ...
    for (pAddr = res, i = 1; pAddr != NULL; pAddr = pAddr->ai_next, i++) {
        ...
        clientSock = WSASocket(pAddr->ai_family, pAddr->ai_socktype, pAddr->ai_protocol, NULL, 0, 0);
        if (clientSock == INVALID_SOCKET) {
            printf("WSASocket failed. Error = %d", WSAGetLastError());printf("\n");
    ...
    CLEANUP:
        ...
        if (res) {
            freeaddrinfo (res);
            printf("Freed the memory allocated for res by getaddrinfo");printf("\n");
        }
        printf("Exiting CreateClientSocket()");printf("\n");
        return clientSock;
}
...
BOOL PrepareSendBuffer() {
    ...
    printf("Entering PrepareSendBuffer()");printf("\n");
    
    g_ClientContext.pSendBuf = (char *)malloc(g_ClientContext.sendBufSize + 1);
    if (g_ClientContext.pSendBuf == NULL) {
        printf("malloc failed."); printf("\n");
        goto CLEANUP;
    }
    
    printf("Allocated Send Buffer: %p", g_ClientContext.pSendBuf);printf("\n");
    ...
    CLEANUP:
        printf("Exiting PrepareSendBuffer()");printf("\n");
        return bSuccess;
}
/* 
...
void FreeSendBuffer() {
    if (g_ClientContext.pSendBuf != NULL) {
        ...
        printf("Freed Send Buffer : %p", g_ClientContext.pSendBuf);printf("\n");
...
BOOL PrepareRecvBuffer() {
    ...
    printf("Entering PrepareRecvBuffer()");printf("\n");
    ...
    printf("Allocated Recv Buffer: %p", g_ClientContext.pRecvBuf);printf("\n");
    ...
    CLEANUP:
        printf("Exiting PrepareRecvBuffer()");printf("\n");
        return bSuccess;
}
...
void FreeRecvBuffer() {
    if (g_ClientContext.pRecvBuf != NULL) {
        ...
        printf("Freed Recv Buffer : %p", g_ClientContext.pRecvBuf);printf("\n");
...
int DoSendOnce() {
    ...
    printf("Entering DoSendOnce()");printf("\n");
    ...
    nBytesSent = send(g_ClientContext.sock, g_ClientContext.pSendBuf + startPosition, g_ClientContext.nBytesRemainingToBeSent, 0);
    ...
    printf("Sent %d bytes so far", startPosition + nBytesSent);printf("\n");
    
    CLEANUP:
        printf("Exiting DoSendOnce()");printf("\n");
        return err;
}
/* 
...
int DoRecvOnce() {
    ...
    printf("Entering DoRecvOnce()");printf("\n");
    ...
    */
    nBytesRecd = recv(g_ClientContext.sock, g_ClientContext.pRecvBuf, g_ClientContext.recvBufSize, 0);
    if (nBytesRecd == SOCKET_ERROR) {
        printf("recv returned: %d", WSAGetLastError());printf("\n");
        goto CLEANUP;
    }
    // update the stats.
    g_ClientContext.nBytesRecd += nBytesRecd;
    printf("Recd %d bytes so far", g_ClientContext.nBytesRecd);printf("\n");
    
    CLEANUP:
        printf("Exiting DoRecvOnce()");printf("\n");
        return nBytesRecd;
}
...
void DoSendUntilDone() {
    ...
    printf("Entering DoSendUntilDone()");printf("\n");
        ...
        switch (err) {
            case 0:
                if (g_ClientContext.nBytesRemainingToBeSent == 0) {
                    printf("Send completed");printf("\n");
                    goto CLEANUP;
                }
                break; 
            case WSAEWOULDBLOCK:
                printf("Got WSAEWOULDBLOCK from send.");printf("\n");
                printf("Waiting for 1 second before retrying send ...");printf("\n");
                Sleep (1000);
                break; 
            default: {
                // other errors.
    ...
    CLEANUP:
        printf("Exiting DoSendUntilDone()");printf("\n");
        return; 
}
/* 
...
void DoRecvUntilDone() {
    ...
    printf("Entering DoRecvUntilDone()");printf("\n");
    
    do {
        // get the job done by calling DoRecvOnce.
        switch (DoRecvOnce()) {
            case 0:
                printf("Recv returned 0. Remote socket must have been gracefully closed.");printf("\n");
                goto CLEANUP;
            case SOCKET_ERROR:
                err = WSAGetLastError();
                if (err == WSAEWOULDBLOCK) {
                    printf("Got WSAEWOULDBLOCK from recv.");printf("\n");
                    printf("Waiting for 1 second before retrying send ...");printf("\n");
                    Sleep (1000);
                }
                else {
                    printf("ERROR: recv returned : %d", err);printf("\n");
                    goto CLEANUP;
                }
                break; 
            default: {
    ...
    while (TRUE);
    CLEANUP:
        printf("Exiting DoRecvUntilDone()");printf("\n");
        return; 
}
/* 
...
void DoShutDown() {
    if (shutdown(g_ClientContext.sock, SD_SEND) == SOCKET_ERROR) {
        printf("shutdown failed. err = %d", WSAGetLastError()); printf("\n");
    }
    else {
        printf("shutdown successful");printf("\n");
    }
}
...
void DoSendWaitRecv() {
    DoSendUntilDone ();
    DoShutDown ();
    printf("Waiting before doing recv ...");printf("\n");
...
*/
void DoWaitSendRecv() {
    printf("Waiting before doing initial send ...");printf("\n");
    Sleep (g_ClientContext.delay);
    DoSendUntilDone ();
    DoShutDown ();
    DoRecvUntilDone ();
...
void DoIdealSendRecv() {
    ...
    if (hThread == NULL) {
        printf("ERROR: CreateThread failed. Error = %d", GetLastError()); printf("\n");
        goto CLEANUP;
    }
    ...
    printf("Recv Thread done.");printf("\n");
...
int __cdecl main(int argc, char *argv[]) {
    ...
    */
    ...
    if (retVal != 0) {
        printf("WSAStartup failed. Error = %d", retVal); printf("\n");
        goto CLEANUP;
    }
    ...
    for (i = 1; i <= g_ClientContext.loopCount; i++) {
        printf("\n\nPerforming Iteration : %d", i);printf("\n");
        printf("---------------------------");printf("\n");
        ...
        switch (g_ClientContext.scenario) {
            ...
            case SC_IDEAL_SEND_RECV:
                ...
                break; 
            default: {
                printf("Unrecognized scenario");printf("\n");
                break; 
            }
        }
    }
    ...
    CLOSE_SOCKET:
        ...
        printf("Closed socket %d. Total Bytes Recd = %d, Total Bytes Sent = %d", g_ClientContext.sock, g_ClientContext.nBytesRecd, g_ClientContext.sendBufSize - g_ClientContext.nBytesRemainingToBeSent);printf("\n");
