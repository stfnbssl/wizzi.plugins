/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.wfschema\lib\artifacts\wfschema\factory\gen\ittf\wfschema-factory.js.ittf
*/
'use strict';
//
...
var wzschemamTreePreProcessor = require('./wzschema-mtree-preprocessor.g');

var wzschemaschema = require('./wzschema-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var wzschemamodel = new wzschemamodelType('EmptyIttfDocument');
        }
        ...
        // Load the WizziModel from the root node of the mTree
            ...
            var wzschemamodelType = wzschemaschema[rootNode.n];
            if (!wzschemamodelType) {
                var maptag = wzschemaschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    wzschemamodelType = wzschemaschema[maptag];
                }
                if (!wzschemamodelType) {
                    var error = new errors.WizziModelLoadError('In wzschema Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: wzschema', ittfDocumentUri);
                    return callback(error);
                }
            }
            wzschemamodel = new wzschemamodelType(rootNode.v);
            wzschemamodel.loadHistory = mTree.loadHistory;
            wzschemamodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                wzschemamodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn wzschema Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new wzschemaschema.wzschemaContext();
        wzschemamodel.wzInitialize(ctx);
        wzschemamodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In wzschema Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        ...
        // dump for debug
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && wzschemamodel.toJson && file.isFilePath(ittfDocumentUri)) {
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(wzschemamodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        wzschemamodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            
            // dump for debug
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && wzschemamodel.toJson && file.isFilePath(ittfDocumentUri)) {
            ...
            callback(null, wzschemamodel);
        })
    }
    
    /**
        * Load a WizziModel of type 'wzschema' from an mTree
    */
    ...
    /**
        // Load a WizziModel of type 'wzschema' from an IttfDocument uri
        // params
    ...
    */
        ...
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema wzschema from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
                    ...
                    mTree = wzschemamTreePreProcessor(mTree, loadContext)
                    ;
