/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.wzschema\.wizzi\examples\wfschema_factory.js.ittf
*/
'use strict';
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'wfschema/factory.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'wfschema/factory.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'wfschema/factory.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('wfschema/factory.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.wfschema.ittf');
            var outputPath = path.join(__dirname, 'results', 'wfschema', module + '-factory.g.js');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'wfschema/factory', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('wfschema/factory.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
