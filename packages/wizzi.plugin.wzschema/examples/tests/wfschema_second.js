/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.wzschema\.wizzi-override\examples\tests\wfschema_second.js.ittf
    utc time: Fri, 15 Mar 2024 08:46:05 GMT
*/
'use strict';
...
function executeExample() {
    ...
    executeLoadIttfDocuments([
        moduleName
    ], (err, result) => {
    
        if (err) {
            console.log("[31m%s[0m", 'wfschema_second.examples.executeLoadIttfDocuments.err', err);
            console.log("[31m%s[0m", 'wfschema_second.examples.executeLoadIttfDocuments.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'wfschema_second.examples.executeLoadIttfDocuments.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeLoadIttfDocuments(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
        
            console.log('wfschema_second.example.executeLoadIttfDocuments.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.second.ittf');
            var outputPath = path.join(__dirname, 'results', module + '.second.json');
            loadMTree(ittfDocumentUri, {}, (err, mTree) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('wfschema_second.example.executeLoadIttfDocuments.mTree: ', mTree, __filename);
                createWizziFactory({}, (err, wf) => {
                
                    if (err) {
                        return callback(err);
                    }
                    var loadModel = secondFactory.createLoadModel({
                        options: {
                            loadFromMTree: true
                         }, 
                        verify: verify, 
                    ...
                    loadModel(mTree, {}, (err, wizziModel) => {
                    
                        if (err) {
                            return callback(err);
                        }
                        console.log('wfschema_second.example.executeLoadIttfDocuments.wizziModel: ', wizziModel, __filename);
...
function executeWizziJob_2(wfjobDocumentUri, options) {
    ...
    wizzi.executeWizziJob({
        ...
        if (err) {
            wizzi.printWizziJobError('wfschema_second', err);
        }
    })
}
