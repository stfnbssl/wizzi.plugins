/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.wzschema\.wizzi-override\examples\tests\wfschema_graphql.js.ittf
...
var graphqlFactory = require('../results/wfschema/graphql-factory.g');
function executeExample() {
    ...
    executeLoadIttfDocuments([
        ...
        if (err) {
            console.log("[31m%s[0m", 'wfschema_graphql.examples.executeLoadIttfDocuments.err', err);
            console.log("[31m%s[0m", 'wfschema_graphql.examples.executeLoadIttfDocuments.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'wfschema_graphql.examples.executeLoadIttfDocuments.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeLoadIttfDocuments(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('wfschema_graphql.example.executeLoadIttfDocuments.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.graphql.ittf');
            var outputPath = path.join(__dirname, 'results', module + '.graphql.json');
            loadMTree(ittfDocumentUri, {}, (err, mTree) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('wfschema_graphql.example.executeLoadIttfDocuments.mTree: ', mTree, __filename);
                createWizziFactory({}, (err, wf) => {
                
                    if (err) {
                        return callback(err);
                    }
                    var loadModel = graphqlFactory.createLoadModel({
                        options: {
                            loadFromMTree: true
                         }, 
                        verify: verify, 
...
function executeWizziJob_2(wfjobDocumentUri, options) {
    ...
    wizzi.executeWizziJob({
        ...
        if (err) {
            wizzi.printWizziJobError('wfschema_graphql', err);
        }
    })
}
