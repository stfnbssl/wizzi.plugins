/*
    ...
    utc time: Fri, 15 Mar 2024 17:19:37 GMT
*/
'use strict';
/**
     first WizziModelFactory
*/
...
var firstschema = require('./first-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var basenodemodel = new basenodemodelType('EmptyIttfDocument');
        }
        ...
        // Load the WizziModel from the root node of the mTree
            ...
            var basenodemodelType = firstschema[rootNode.n];
            if (!basenodemodelType) {
                var maptag = firstschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    basenodemodelType = firstschema[maptag];
                }
                if (!basenodemodelType) {
                    var error = new errors.WizziModelLoadError('In first Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: first', ittfDocumentUri);
                    return callback(error);
                }
            }
            basenodemodel = new basenodemodelType(rootNode.v);
            basenodemodel.loadHistory = mTree.loadHistory;
            basenodemodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                basenodemodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn first Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new firstschema.firstContext();
        basenodemodel.wzInitialize(ctx);
        basenodemodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In first Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        ...
        // dump for debug
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && basenodemodel.toJson && file.isFilePath(ittfDocumentUri)) {
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(basenodemodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        basenodemodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            
            // dump for debug
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && basenodemodel.toJson && file.isFilePath(ittfDocumentUri)) {
            ...
            callback(null, basenodemodel);
        })
    }
    
    /**
        * Load a WizziModel of type 'first' from an mTree
    */
    ...
    /**
        // Load a WizziModel of type 'first' from an IttfDocument uri
        // params
    ...
    */
        ...
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema first from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
