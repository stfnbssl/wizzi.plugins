/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.wzschema\.wizzi-override\examples\wzschema_html_docs.js.ittf
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'wzschema/html_docs.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'wzschema/html_docs.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'wzschema/html_docs.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, wzCtx, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputPath = path.join(__dirname, 'results', 'wzschema', module + '-html_docs.g.html');
            loadWizziModel(ittfDocumentUri, {
                ...
                loadModelAndGenerateArtifact(ittfDocumentUri, {
                    wzCtx: wzCtx
                 }, 'wzschema/html_docs', (err, artifactText) => {
