/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.wzschema\.wizzi\examples\wfschema_text.js.ittf
*/
'use strict';
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'wfschema/text.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'wfschema/text.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'wfschema/text.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('wfschema/text.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.wfschema.ittf');
            var outputPath = path.join(__dirname, 'results', 'wfschema', module + '-text.g.js');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'wfschema/text', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('wfschema/text.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
