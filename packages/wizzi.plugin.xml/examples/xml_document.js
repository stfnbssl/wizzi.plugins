/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.xml\.wizzi-override\examples\xml_document.js.ittf
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'xml/document.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'xml/document.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'xml/document.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('xml/document.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.xml.ittf');
            var outputPath = path.join(__dirname, 'results', 'xml', module + '-document.g.xml');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'xml/document', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('xml/document.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
