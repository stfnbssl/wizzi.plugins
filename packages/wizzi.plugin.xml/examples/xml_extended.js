/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.xml\.wizzi-override\examples\xml_extended.js.ittf
    utc time: Thu, 25 Apr 2024 11:41:30 GMT
*/
'use strict';
...
function executeExample() {
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'xml.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'xml.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'xml.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('xml.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.xml.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.xml.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.xml.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('xml.example.executeGenerateModules.outputLoadedPath: ' + outputLoadedPath, __filename);
                if (model.toJson && verify.isFunction(model.toJson)) {
                    file.write(outputLoadedPath, stringify(model.toJson(), null, 4))
                }
                loadModelAndTransform(ittfDocumentUri, {}, "xml/extended", (err, model) => {
                
                    if (err) {
                        return callback(err);
                    }
                    console.log('xml.example.executeGenerateModules.outputExtendedPath: ' + outputExtendedPath, __filename);
...
function executeWizziJob_2(wfjobDocumentUri, options) {
    ...
    wizzi.executeWizziJob({
        ...
        if (err) {
            wizzi.printWizziJobError('xml', err);
        }
    })
}
