...
var firstmTreePreProcessor = require('./first-mtree-preprocessor.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var firstmodel = new firstmodelType('EmptyIttfDocument');
        }
        ...
        // Load the WizziModel from the root node of the mTree
            ...
            firstmodel = new firstmodelType(rootNode.v);
            firstmodel.loadHistory = mTree.loadHistory;
            firstmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                firstmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
        ...
        firstmodel.wzInitialize(ctx);
        firstmodel.wzVerify(ctx);
        ...
        // dump for debug
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && firstmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(firstmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        firstmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            
            // dump for debug
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && firstmodel.toJson && file.isFilePath(ittfDocumentUri)) {
    ...
    */
        ...
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    mTree = firstmTreePreProcessor(mTree, loadContext)
                    ;
