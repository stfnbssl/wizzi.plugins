/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.first\.wizzi\examples\first_extended.js.ittf
    utc time: Fri, 07 Apr 2023 14:53:54 GMT
*/
'use strict';
...
function executeExample() {
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'first.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'first.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'first.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.first.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.first.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.first.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                ...
                loadModelAndTransform(ittfDocumentUri, {}, "first/extended", (err, model) => {
...
function executeWizziJob_2(wfjobDocumentUri, options) {
    ...
    wizzi.executeWizziJob({
        ...
        if (err) {
            wizzi.printWizziJobError('first', err);
        }
    })
}
