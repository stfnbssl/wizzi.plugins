/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.first\.wizzi\examples\first_document.js.ittf
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'first/document.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'first/document.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'first/document.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputPath = path.join(__dirname, 'results', 'first', module + '-document.g.html');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'first/document', (err, artifactText) => {
