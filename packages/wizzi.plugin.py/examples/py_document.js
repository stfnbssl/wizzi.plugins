/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.py\.wizzi-override\examples\py_document.js.ittf
*/
'use strict';
...
function executeExample() {
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputPath = path.join(__dirname, 'results', module + '.py.html');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'py/document', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('py.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
