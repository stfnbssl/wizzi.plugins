/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.svg\.wizzi-override\examples\svg_extended.js.ittf
    utc time: Sun, 12 May 2024 13:34:20 GMT
*/
'use strict';
...
function executeExample() {
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputLoadedPath = path.join(__dirname, 'results', module + '.svg.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.svg.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('svg.example.executeGenerateModules.outputLoadedPath: ' + outputLoadedPath, __filename);
                file.write(outputLoadedPath, stringify(model.toJson(), null, 4))
                loadModelAndTransform(ittfDocumentUri, {}, "svg/extended", (err, model) => {
                
                    if (err) {
                        return callback(err);
                    }
                    console.log('svg.example.executeGenerateModules.outputExtendedPath: ' + outputExtendedPath, __filename);
