/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.bin\.wizzi-override\examples\bin_document.js.ittf
...
function executeExample() {
    ...
    getWzCtx(path.resolve(__dirname, '..', '.wizzi-override', 'models'), (err, wzCtx) => {
        ...
        executeGenerateModules([
            ...
            if (err) {
                console.log("[31m%s[0m", 'bin/document.examples.executeGenerateModules.err', err);
                console.log("[31m%s[0m", 'bin/document.examples.executeGenerateModules.err.toString()', err.toString());
                if (err.inner) {
                    console.log("[31m%s[0m", 'bin/document.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
                }
            }
            else {
            }
    ...
    function executeGenerateModules(modules, wzCtx, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.bin.ittf');
            var outputPath = path.join(__dirname, 'results', 'bin', module + '.bin');
            loadWizziModel(ittfDocumentUri, {
                ...
                loadModelAndGenerateArtifact(ittfDocumentUri, {
                    wzCtx: wzCtx
                 }, 'bin/document', (err, artifactText) => {
                
                    if (err) {
                        return callback(err);
                    }
                    console.log('artifactText.length', artifactText.length, __filename);
                    var dataBase64 = Buffer.from(artifactText,'base64');
                    try {
                        fs.writeFile(outputPath, dataBase64, {
                            encoding: 'base64'
                         }, (err, notUsed) => {
