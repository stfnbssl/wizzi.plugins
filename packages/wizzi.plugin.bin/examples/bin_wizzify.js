/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.bin\.wizzi-override\examples\bin_wizzify.js.ittf
...
function executeExample() {
    ...
    function wizzify(name, callback) {
        ...
        var testSourcePath = path.join(__dirname, 'data', name + '.test.ico');
        
        createWizziFactory({}, (err, wf) => {
        
            // TEST
            // Create a base64 string from an image => ztso+Mfuej2mPmLQxgD ...
            const testSource = fs.readFileSync(source, "base64");
            console.log('testSource.length', testSource.length, __filename);
            // Convert base64 to buffer => <Buffer ff d8 ff db 00 43 00 ...
            const testSourceBuffer = Buffer.from(testSource, "base64");
            // Pipes an image with "new-path.jpg" as the name.
            fs.writeFileSync(testSourcePath, testSourceBuffer)
            binwizzifier.getWizziIttf(fs.readFileSync(source, "base64"), {
                wf: wf, 
                ...
                file.write(path.join(__dirname, 'data', 'output', name + '.bin.ittf'), ittf)
