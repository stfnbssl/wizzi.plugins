/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.json\.wizzi-override\examples\json_extended.js.ittf
    utc time: Thu, 23 Jan 2025 10:12:13 GMT
*/
...
function executeExample() {
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'json.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'json.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'json.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            console.log('json.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.json.ittf');
            var outputLoadedPath = path.join(__dirname, 'results', module + '.json.json');
            var outputTransformedPath = path.join(__dirname, 'results', module + '.json.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                if (err) {
                    return callback(err);
                }
                console.log('json.example.executeGenerateModules.outputLoadedPath: ' + outputLoadedPath, __filename);
                if (model.toJson && verify.isFunction(model.toJson)) {
                    file.write(outputLoadedPath, stringify(model.toJson(), null, 4))
                }
                loadModelAndTransform(ittfDocumentUri, {}, "json/extended", (err, model) => {
                    if (err) {
                        return callback(err);
                    }
                    console.log('json.example.executeGenerateModules.outputTransformedPath: ' + outputTransformedPath, __filename);
                    file.write(outputTransformedPath, stringify(model, null, 4))
...
function executeWizziJob_2(wzjobDocumentUri, options) {
    ...
    wizzi.executeWizziJob({
        ...
        if (err) {
            wizzi.printWizziJobError('json', err);
        }
    })
}
