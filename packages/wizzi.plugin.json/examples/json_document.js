...
function executeExample() {
    ...
    console.log(arg, moduleName, __filename);
    executeGenerateModules([
        moduleName
    ], (err, result) => {
        if (err) {
            console.log("[31m%s[0m", 'json/document.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'json/document.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'json/document.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputPath = path.join(__dirname, 'results', 'json', module + '-document.g.json');
            console.log('outputPath: ' + outputPath, __filename);
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'json/document', (err, artifactText) => {
                if (err) {
                    return callback(err);
                }
                console.log('artifactText: ' + artifactText, __filename);
                console.log('json/document.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
                file.write(outputPath, artifactText)
                if (moduleName == "iot-data") {
                    ittfDocumentUri = path.join(__dirname, 'ittf', 'iot.json.ittf')
                    ;
                    outputPath = path.join(__dirname, 'results', 'json', 'iot-document.g.json')
                    ;
                    loadModelAndGenerateArtifact(ittfDocumentUri, {
                        data: JSON.parse(artifactText)
                     }, 'json/document', (err, artifactText) => {
                        if (err) {
                            return callback(err);
                        }
                        file.write(outputPath, artifactText)
