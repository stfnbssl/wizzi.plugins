...
'use strict';
/**
     regexp WizziModelFactory
*/
var path = require('path');
var util = require('util');
var _ = require('lodash');
var stringify = require('json-stringify-safe');
var regexpschema = require('./regexp-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var regexpmodel = new regexpmodelType('EmptyIttfDocument');
        }
        else {
            // Get the model type of the root node of the ittf model.
            var rootNode = mTree.nodes[0];
            var regexpmodelType = regexpschema[rootNode.n];
            if (!regexpmodelType) {
                var maptag = regexpschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    regexpmodelType = regexpschema[maptag];
                }
                if (!regexpmodelType) {
                    var error = new errors.WizziModelLoadError('In regexp Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: regexp', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            regexpmodel = new regexpmodelType(rootNode.v);
            regexpmodel.loadHistory = mTree.loadHistory;
            regexpmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                regexpmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn regexp Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new regexpschema.regexpContext();
        regexpmodel.wzInitialize(ctx);
        regexpmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In regexp Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        // _ log.info('Initialized wmt model ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && regexpmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(regexpmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        regexpmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && regexpmodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
            ...
            callback(null, regexpmodel);
        });
    }
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'regexp' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'regexp' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema regexp from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
