/*
    ...
    utc time: Fri, 01 Jul 2022 14:56:03 GMT
*/
'use strict';
/**
     docx WizziModelFactory
*/
var path = require('path');
var util = require('util');
var _ = require('lodash');
var stringify = require('json-stringify-safe');
var docxmTreePreProcessor = require('./docx-mtree-preprocessor.g');
var docxschema = require('./docx-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var docxmodel = new docxmodelType('EmptyIttfDocument');
        }
        else {
            // Get the model type of the root node of the ittf model.
            var rootNode = mTree.nodes[0];
            var docxmodelType = docxschema[rootNode.n];
            if (!docxmodelType) {
                var maptag = docxschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    docxmodelType = docxschema[maptag];
                }
                if (!docxmodelType) {
                    var error = new errors.WizziModelLoadError('In docx Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: docx', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            docxmodel = new docxmodelType(rootNode.v);
            docxmodel.loadHistory = mTree.loadHistory;
            docxmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                docxmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn docx Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new docxschema.docxContext();
        docxmodel.wzInitialize(ctx);
        docxmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In docx Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        // _ log.info('Initialized wmt model ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && docxmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(docxmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        docxmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && docxmodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
            ...
            callback(null, docxmodel);
        });
    }
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'docx' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'docx' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema docx from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
                    ...
                    mTree = docxmTreePreProcessor(mTree, loadContext);
