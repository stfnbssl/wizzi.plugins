/*
    ...
    utc time: Wed, 13 Mar 2024 07:01:25 GMT
*/
'use strict';
...
function executeExample() {
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'docx.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'docx.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'docx.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            var outputLoadedPath = path.join(__dirname, 'results', module + '.docx.json');
            var outputExtendedPath = path.join(__dirname, 'results', module + '.docx.extended.json');
            loadWizziModel(ittfDocumentUri, {}, (err, model) => {
                ...
                loadModelAndTransform(ittfDocumentUri, {}, "docx/extended", (err, model) => {
