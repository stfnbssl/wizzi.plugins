/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.plugins\packages\wizzi.plugin.ittf\.wizzi-override\examples\ittf_document.js.ittf
    utc time: Tue, 11 Apr 2023 19:36:13 GMT
*/
'use strict';
...
function executeExample() {
    ...
    executeGenerateModules([
        ...
        if (err) {
            console.log("[31m%s[0m", 'ittf/document.examples.executeGenerateModules.err', err);
            console.log("[31m%s[0m", 'ittf/document.examples.executeGenerateModules.err.toString()', err.toString());
            if (err.inner) {
                console.log("[31m%s[0m", 'ittf/document.examples.executeGenerateModules.err.inner.toString()', err.inner.toString());
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (module, callback) => {
            ...
            console.log('ittf/document.example.executeGenerateModules.module: ' + module, __filename);
            var ittfDocumentUri = path.join(__dirname, 'ittf', module + '.ittf.ittf');
            var outputPath = path.join(__dirname, 'results', 'ittf', module + '-document.g.ittf');
            loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'ittf/document', (err, artifactText) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('ittf/document.example.executeGenerateModules.outputPath: ' + outputPath, __filename);
...
function executeWizziJob_2(wfjobDocumentUri, options) {
    ...
    wizzi.executeWizziJob({
        ...
        if (err) {
            wizzi.printWizziJobError('ittf', err);
        }
    })
}
